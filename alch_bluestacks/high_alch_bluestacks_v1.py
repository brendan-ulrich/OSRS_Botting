...
high_alch_icon = [235,363] #high alch icon (located in magic tab)
...
while alchs < num_alchs:
    ...
    magic_tab_open = pyautogui.locateCenterOnScreen(r'alch_bluestacks\alch_images\magic_tab_open.png', confidence=0.95) #if image is not found this will return None
    #print(type(magic_tab_open))
    if magic_tab_open != None: #if magic tab is open
        pyautogui.click(high_alch_icon) #click high alch icon
        ...
        inv_tab_open = pyautogui.locateCenterOnScreen(r'alch_bluestacks\alch_images\inventory_tab_open.png', confidence=0.95) #if image is not found this will return None
        if inv_tab_open != None: #if inventory tab is open
            ...
            sleep_time = (random.randint(15,17))/10 #generate a random time between 0.6 and 0.8 seconds
